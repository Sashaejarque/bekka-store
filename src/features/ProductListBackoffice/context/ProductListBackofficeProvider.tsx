...
export const ProductsListBackofficeProvider: FC<PropsWithChildren> = ({
  children,
}) => {
  ...
  return (
    <ProductsListBackofficeContext.Provider value={values}>
      {children}
...
export const useProductListBackofficeContext = () => {
  ...
  if (!context) {
    throw new Error(
      'useProductList must be used within a ProductlistBackofficeContext'
    );
  }
  return context;
};
